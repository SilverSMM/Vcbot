...
async def fetch_youtube_link(query):
    try:
        url = f"https://heinhtetaungsearch-paid.noobop00111.workers.dev/search?q={query}"
...
async def start_handler(_, message):
    ...
    caption = (
        ...
        "┗────────────────────┛\n"
        f"๏ ᴄʟɪᴄᴋ ᴏɴ ᴛʜᴇ {help_text} ʙᴜᴛᴛᴏɴ ᴛᴏ ɢᴇᴛ ɪɴғᴏʀᴍᴀᴛɪᴏɴ ᴀʙᴏᴜᴛ ᴍʏ ᴍᴏᴅᴜʟᴇs ᴀɴᴅ ᴄᴏᴍᴍᴀɴᴅs."
    )

    buttons = [
        [
            InlineKeyboardButton(f"➕ {add_me_text}", url=f"{BOT_LINK}?startgroup=true"),
            InlineKeyboardButton(f"📢 {updates_text}", url="https://t.me/S03US")
        ],
        [
            InlineKeyboardButton(f"💬 {support_text}", url="https://t.me/+_s54XwEkfc4xZmVl"),
...
async def go_back_callback(_, callback_query):
    ...
    raw_name = callback_query.from_user.first_name or "there"
    styled_name = to_bold_unicode(raw_name)
    user_link = f"[{styled_name}](tg://user?id={user_id})"
